...
description: 将开发任务与基础工作派发给 Codex CLI 执行，Codex 保留体系裁决与门禁。Codex 直接读 .Codex/ 权威源（AGENTS.md、rules/、references/），适用于开发任务、编译错误批量修复、样板代码、批量重命名、格式化、独立小任务。涉及 Assets/Mine/ 的产出经 Codex review + 门禁链合入。触发词：codex, 派发, 编排, 批量修复, 开发, 落地, worker。链路为 OpenCode Go 直连（2026-08-17 起无本地代理）。
---

# Codex 编排 — 派发基础工作

> 单一知识体系（`.Codex/`），双开发引擎。Codex 管体系裁决与门禁，Codex 管开发落地（同样执行开发任务，直接读 .Codex/ 权威源）。

## 架构

```
Codex ──Bash──▶ codex exec --sandbox workspace-write "<任务>"
                        │
                        ▼
...
> 🌍 **全局 skill**：本 skill 是全局的（`~/.Codex/skills/`），任何项目可用。配置在 `~/.codex/`（config.toml + auth.json）也是全局的，无需每项目配置；codex exec 在未信任目录也能直接跑（自动 trust）。知识模型：Codex **直接读 .Codex/ 权威源**（入口指引在 .codex/AGENTS.md，结构变更时由 codex-bridge skill 更新）。
...
4. **并发安全**：多个 Codex+codex 工作流可并行（直连无共享进程、配置只读共享、codex 会话独立）。唯一禁忌：同一 git 仓库内多个工作流写文件会互相冲突，应避免
...
| 类别 | 原因 |
|------|------|
| `.Codex/` 体系变更（宪法/rules/agents/skills 修改） | 体系裁决归 Codex（meta-developer）；Codex 可读但不可改体系 |
| 门禁裁决（Assets/Mine/ 合入） | Codex 无 MCP write_gated 通道；开发产出经 Codex review + 门禁链合入 |
| 渲染管线/架构级方案决策 | 方案设计归 Codex/auto-developer 内部；Codex 按任务书落地 |
| 涉及外部服务的操作 | 发布、推送、部署 |
...
- **自包含**：Codex 直接读 `.Codex/` 权威源（入口指引 `.codex/AGENTS.md` 列清单：AGENTS.md 宪法、rules/、references/）；关键约束仍须写进指令（命名规范、文件路径、验收标准）
...
Codex 的产出**必须**回到 Codex review（`git diff`），问题由 Codex 修或回退。产出合入前先跑规范检查：`python .mcp/validation/check_norm.py <file>`（与 write_gated 同一检查，exit 1 = 有 error 违规需修正）。
...
2. `git diff` 抽查关键文件
3. 不符合 → Codex 直接修或 `git checkout -- <file>` 回退
4. 有编译/测试条件的项目，跑编译验证
...
| Codex 不了解项目规范 | 指令里指明读取 `.Codex/` 权威文件（AGENTS.md / rules/ / references/）；入口清单结构变更后由 codex-bridge 更新 |
...
- `codex-bridge`：双边契约（.codex/INTERFACE.md）+ AGENTS.md 入口同步（低频，结构变更时）——知识本身 Codex 直接读 .Codex/，无需同步
