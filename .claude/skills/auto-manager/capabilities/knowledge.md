...
AutoMode 激活
  ...
  │     ├── 中优先级（按需）：模板类 → ../../templates/standard/ and ../../templates/script/、../../templates/shader/
...
| 用户指令明确涉及普通 Shader | 必读 standard/shader/shader-structure.md + templates/standard/shader/shader-doc-template.md |
| 用户指令明确涉及纯 C# 脚本 | 必读 standard/script/script-structure.md + templates/script/ |
...
4. **文档模板**：按实际功能选择 `templates/standard/`、`templates/script/` 或 `templates/shader/` 下的模板
...
```
写普通 Script
  └── [R1] references/standard/script/
        └── script-structure.md

写普通 Shader / HLSL
  └── [R1] references/standard/shader/
        ├── shader-structure.md
        └── hlsl-includes.md

写 Compute / RenderGraph 基础
  └── [R1] references/standard/compute/ 或 standard/rendering/

写后处理 Shader 功能
  ├── [R1] references/shader/postprocess/
  │     ├── fullscreen-structure.md
  │     ├── blit-fullscreen.md
  │     ├── postprocess-differences.md
  │     └── feature-script-structure.md
  │
  ├── [R2] 复制模板: ../../templates/shader/postprocess/
  │     ├── fullscreen-postprocess.shader
  │     ├── urp-renderpass.cs
  │     ├── volume-template.cs
  │     └── compute-template.compute
  │
  └── [R3] 跨领域平台知识: references/platform/metal-notes.md
```
...
| 条件 | 行为 |
|------|------|
| 写普通 Shader | 读取 `standard/shader/`，按需复制 `templates/standard/shader/` |
| 写纯 C# 脚本 | 读取 `standard/script/`，按职责检查 `templates/script/` |
...
| 标准知识库 | agents/unity-developer/references/standard/ | 跨功能的 Script / Shader / Compute / Rendering 基础 | 按需读取 |
| Shader 功能知识库 | agents/unity-developer/references/shader/ | 后处理等渲染功能及其混合文件类型知识 | 按功能读取 |
| 模板 | agents/unity-developer/templates/standard/、script/、shader/ | 新建文件复制的模板 | 按需复制 |
